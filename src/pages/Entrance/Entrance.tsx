...
const Entrance: React.FC = () => {
  ...
  const [coordinates, setCoordinates] = useState<[number, number, number]>([-13, -2.6, 0]);
  ...
  return (
    showGallery ? (
      <>
        <div className="w-full h-screen flex flex-col items-center overflow-hidden">
          <PhotoRoom setImageData={setImageData} imageData={imageData}>
            <h1 className='text-center font-bold text-2xl m-2'>Galareya xonasi</h1>
      ...
      <>
        ...
        < main className={`w-full h-screen transition-all duration-500`
          ...
          <div className='flex items-center gap-2 fixed top-4 right-4 z-90'>
            ...
            <ShinyText className='h-10 bg-transparent hover:bg-white/10 transition-all duration-300'>
              <div className='flex items-center gap-2 text-white text-[14px] w-full h-full' onClick={() => setShowGallery(true)}>
        ...
        {!showEntrance && (
          <>
            < EnhancedTimelineNav items={navItems} betweenDots={8} coordinates={coordinates} />
          </>
        )}
